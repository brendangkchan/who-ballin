...
| **`npm run fill-map`** | Requires a prior complete `build-map` run. Fetches NBA commonallplayers, matches BDL players missing from the map by name, adds new mappings, and writes any still-unmatched names to `scripts/manual-nba-ids.json` (BDL id + name) for manual fill-in. No API key. | After `build-map` to fill gaps; run again after adding manual IDs to reduce the manual list. |
| **`npm run apply-manual-ids`** | Reads `scripts/manual-nba-ids.json`, copies every entry that has `nba_id` set into `src/lib/nba-player-id-map.json`, then removes those entries from the manual file so only unfilled rows remain. | After you have filled in `nba_id` for some rows in the manual file. |
